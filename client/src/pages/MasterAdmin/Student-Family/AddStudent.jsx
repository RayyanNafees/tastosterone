...
const AddStudent = () => {
  ...
  // const family_email = useRef(null)
  ...
  const handleSubmit = (e) => {
    ...
    if (email_is_valid) {
      ...
      const parent = localStorage.getItem('familyID')
      ...
      axios
        .post(`${baseURL}${'student/'}`, newStudentInfo, config)
        .then((resp) => {
          console.log(resp)
          if (resp.status === 201) {
            const data = resp.data
            navigate(`/master-admin/student-profile/${data.id}`)
          }
        })
        .catch((err) => {
          console.log(err)
          if (err.response.data['email']) {
            ...
            refs[index].current.placeholder = 'email has been used'
